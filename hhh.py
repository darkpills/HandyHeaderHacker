...
def ReferrerPolicy(searchheaders):
	for headerName, headerValue in searchheaders:
		if "Referrer-Policy".lower() in headerName.lower():
...
def XFrameOptions(searchheaders):
	for headerName, headerValue in searchheaders:
		if "X-Frame-Options".lower() in headerName.lower():
			print("\033[1;32m[+]\033[0m Detected X-Frame-Options - '" + headerValue.rstrip() + "' \033[1;32m(OK)\033[0m")
			return
	print("\033[1;31m[-]\033[0m X-Frame-Options not present \033[1;31m(Not OK)\033[0m"		)
def ContentSecurityPolicy(searchheaders):
	detected = False
	for headerName, headerValue in searchheaders:
		if "Content-Security-Policy".lower() in headerName.lower():
			print("\033[1;32m[+]\033[0m Detected Content-Security-Policy - '" + headerValue.rstrip() + "' \033[1;32m(OK)\033[0m")
			detected = True
		if "X-Webkit-CSP".lower() in headerName.lower():
			print("\033[1;32m[+]\033[0m Detected X-Webkit-CSP - '" + headerValue.rstrip() + "' \033[1;32m(OK)\033[0m")
			detected = True
		if "X-Content-Security-Policy".lower() in headerName.lower():
			print("\033[1;32m[+]\033[0m Detected X-Content-Security-Policy - '" + headerValue.rstrip() + "' \033[1;32m(OK)\033[0m")
			detected = True

	#Report Only
		if "Content-Security-Policy-Report-Only".lower() in headerName.lower():
			print("\033[1;33m[I]\033[0m Detected Content-Security-Policy in report only - '" + headerValue.rstrip() + "' \033[1;33m(Informational)\033[0m")
			detected = True
		if "X-Webkit-CSP-Report-Only".lower() in headerName.lower():
			print("\033[1;33m[I]\033[0m Detected X-Webkit-CSP in report only - '" + headerValue.rstrip() + "' \033[1;33m(Informational)\033[0m")
			detected = True
		if "X-Content-Security-Policy-Report-Only".lower() in headerName.lower():
			print("\033[1;33m[I]\033[0m Detected X-Content-Security-Policy in report only  - '" + headerValue.rstrip() + "' \033[1;33m(Informational)\033[0m")
			detected = True
	#Any headers?
	if detected is True:
		return
...
def XXSSProtection(searchheaders):
	for headerName, headerValue in searchheaders:
		if "X-XSS-Protection".lower() in headerName.lower():
			print("\033[1;32m[+]\033[0m Detected X-XSS-Protection - '" + headerValue.rstrip() + "' \033[1;32m(OK)\033[0m")
			return
	print("\033[1;31m[-]\033[0m X-XSS-Protection not present \033[1;31m(Not OK)\033[0m")
def XContentTypeOptions(searchheaders):
	for headerName, headerValue in searchheaders:
		if "X-Content-Type-Options".lower() in headerName.lower():
			print("\033[1;32m[+]\033[0m Detected X-Content-Type-Options - '" + headerValue.rstrip() + "' \033[1;32m(OK)\033[0m")
			return
...
def GeneralInspect(searchheaders):
	serverversion = ""
	for headerName, headerValue in searchheaders:
		if "Server".lower() in headerName.lower() and headerName.startswith("Server:"):
			print("\033[1;33m[I]\033[0m Detected Server header - '" + headerValue.rstrip() + "' \033[1;33m(Informational)\033[0m")
			serverversion = headerValue
		if "ETag".lower() in headerName.lower():
			if "Apache".lower() in serverversion.lower():
				print("\033[1;33m[I]\033[0m Detected ETag Apache - '" + headerValue.rstrip() + "' \033[1;33m(Informational)\033[0m")
			else:
				print("\033[1;34m[I]\033[0m Possible ETag - '" + headerValue.rstrip() + "' \033[1;34m(Possible Informational)\033[0m")
		if "X-Powered-By".lower() in headerName.lower():
			print("\033[1;33m[I]\033[0m Detected X-Powered-By - '" + headerValue.rstrip() + "' \033[1;33m(Informational)\033[0m")

def CookieInspection(searchheaders):
	for headerName, headerValue in searchheaders:
		if "Set-Cookie".lower() in headerName.lower():
			CookieSplit = headerValue.split(';')
			del CookieSplit[0]
			CookieSplit[-1] = CookieSplit[-1].rstrip()
			CookieString = ''.join(CookieSplit)
			if "HttpOnly".lower() not in CookieString.lower():
				print("\033[1;31m[-]\033[0m Cookie not marked HttpOnly - '" + headerValue.rstrip() + "' \033[1;31m(Not OK)\033[0m")
			if "Secure".lower() not in CookieString.lower():
				print("\033[1;31m[-]\033[0m Cookie not marked Secure - '" + headerValue.rstrip() + "' \033[1;31m(Not OK)\033[0m")
def SecureChecks(searchheaders):
	headerlist = {str(headerName).lower(): str(headerValue) for headerName, headerValue in searchheaders}
	if "Strict-Transport-Security".lower() in headerlist:
		print("\033[1;32m[+]\033[0m Detected Strict-Transport-Security - " + headerlist["strict-transport-security"] + "' \033[1;32m(OK)\033[0m")
	else:
		print("\033[1;31m[-]\033[0m Strict-Transport-Security not present \033[1;31m(Not OK)\033[0m")
	if "Public-Key-Pins".lower() in headerlist:
		print("\033[1;32m[+]\033[0m Detected Public-Key-Pins - " + headerlist["public-key-pins"] + "' \033[1;32m(OK)\033[0m")
	else:
...
def AnomalousHeaders(searchheaders):
	...
	for headerName, headerValue in searchheaders:
		if not any(y.lower() in headerName.lower() for y in KnownHeaders):
			print("\033[1;34m[I]\033[0m Anomalous Header detected '" + headerName.rstrip() + "' \033[1;34m(Possible Informational)\033[0m")
...
def RetrieveHeader(Target, **cookiekey):
	...
	if "https" in Target[:5]:
		...
		try:
			...
			RequestFormed = urllib.request.Request(Target, headers=Cookies)
			ReplyHeaders = urllib.request.urlopen(RequestFormed, context=sslcontext).getheaders()
		...
		if len(CookieGlobal) > 0:
			print("Using provided cookie...")
			RequestFormed = urllib.request.Request(Target,  headers={"Cookie" : CookieGlobal})
			ReplyHeaders = urllib.request.urlopen(RequestFormed).getheaders()
		else:
			ReplyHeaders = urllib.request.urlopen(Target).getheaders()
		return ReplyHeaders
